"""Plotting configuration and drawing utilities for gamma-ray catalogs.

The module is organized in two layers:

1. **Configuration** – :data:`CATALOG_STYLES` defines the visual identity of
   each catalog (marker, color, size, zorder), and ``DEFAULT_MIN_EXTENSION``
   sets the smallest extension that gets a dashed circle.

2. **Transform + drawing layer** – :class:`SkyPlotter` wraps a matplotlib
   Axes and hides the differences between plain Axes and WCSAxes:
   ...
   Point sources are scattered with the per-catalog marker styles, extended
   sources get an additional dashed circle with their angular radius, and
   every source can be labeled with its catalog name using white-halo text.
"""
...
DEFAULT_MIN_EXTENSION = 0.1 * u.Unit("deg")
