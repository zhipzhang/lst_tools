...
def select_configured_dl3_products(
    ...
    """Select exact configured products, rejecting ambiguous source matches."""
    available_by_key = {}
    for product in available:
        if product.key in available_by_key:
            other = available_by_key[product.key]
            raise RuntimeError(
                f"Multiple DL3 files match {product.key}: {other.path} and {product.path}"
            )
        available_by_key[product.key] = product
